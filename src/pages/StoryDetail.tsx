...
const StoryDetail = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      navigate("/search?intent=stories");
    } catch (err: any) {
  ...
  useEffect(() => {
    const cfg = window.TAPNE_RUNTIME_CONFIG;
    if (!cfg?.api?.blogs || !storyId) { setLoading(false); return; }
    ...
    return (
      <div className="flex min-h-screen flex-col">
        ...
        <main className="flex flex-1 flex-col items-center justify-center px-4 text-center">
          ...
          <Button className="mt-4" onClick={() => navigate("/search?intent=stories")}>Back to stories</Button>
        </main>
        <Footer />
      </div>
    );
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="flex-1">
        <article className="mx-auto max-w-[700px] px-4 py-8">
          ...
          <div
            className="prose prose-neutral dark:prose-invert max-w-none text-foreground/90 [&_img]:my-4 [&_img]:rounded-lg"
